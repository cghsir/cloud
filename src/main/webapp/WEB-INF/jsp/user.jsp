...
<shiro:hasPermission name="user:admin">user:admin权限!!!!!</shiro:hasPermission>
...
<ul>
    <c:forEach items="${userList }" var="user">
        <li>用户名：${user.name }----密码：${user.password }----
            <shiro:hasPermission name="sys:edit">
                <a href="${pageContext.request.contextPath }/user/edit/${user.id}">修改用户（测试根据不同用户可访问权限不同，本例tom无权限，jack有权限）</a></li>
            </shiro:hasPermission>
    </c:forEach>
</ul>
